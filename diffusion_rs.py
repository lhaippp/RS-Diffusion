...
class RS_Block(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.is_sliding:
            # 如果不滑动，那么就是正常按照patch_rows 对行进行分组，分成patch_nums的patch
            patch_nums = H // self.rows
            x = x.view(B, C, patch_nums, self.rows, W).contiguous()
            N = self.rows * W
            feat = x
        else:
            # 如果滑动，那么就是将整个行往上滑动 self.rows//2行，掐头去尾进行分组self attention
            H = H - self.rows
        ...
        attn = (q @ k.transpose(-2, -1)) * self.scale #+  self.pe.view(1, 1, 1, 1, -1) # 这个PE需要考虑一下加不加，感觉可以不用加
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, x, time_emb = None, class_emb = None, condition = None):
        ...
        if exists(self.mlp) and (exists(time_emb) or exists(class_emb)):
            ...
            cond_emb = torch.cat(cond_emb, dim = -1) #把time和condition拼在一起 (32，512)
            cond_emb = self.mlp(cond_emb)#(32，512)-(32 128)
            cond_emb = rearrange(cond_emb, 'b c -> b c 1 1')#(32 128)-(32 128,1,1)
            scale_shift = cond_emb.chunk(2, dim = 1)# len 2 每一个shape 为(32 64,1,1)
...
class Unet(nn.Module):
    def __init__(
        ...
        # layers
        self.rs1 = RS_Block(dim=64, is_sliding=False)
        self.rs2 = RS_Block(dim=64, is_sliding=True)
    ...
    def forward(
        ...
        #x = torch.cat((x, condition), dim=1)  #改
        #x = self.conv1(x)
        ...
        x = self.init_conv(x)  #([32, 3, 32, 32])-([32, 64, 32, 32])
        x = self.init_conv1(x)
        #condition = positional_encoding(condition,num_encoding_functions=4, include_input=True, log_sampling=True)
        condition = self.init_conv1(condition) #([32, 3, 32, 32])-([32, 64, 32, 32])

        r = x.clone()
        r_c = condition.clone()

        t = self.time_mlp(time)#([32]) - ([32, 256])


        
        h = []
        con = []
        for block1, block2, attn, downsample in self.downs: #每一层unet有两个resnet block,接一个attention block,最后再下采样
...
class GaussianDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def ddim_sample(self, classes, condition,shape, cond_scale = 1., rescaled_phi = 0, clip_denoised = True):#改
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(img, time_cond, classes, condition,cond_scale = cond_scale, clip_x_start = clip_denoised)#改
    ...
    @torch.no_grad()
    def sample(self, classes, condition, cond_scale=6., rescaled_phi=0.7):  #改
        ...
        return sample_fn(classes, condition,(batch_size, channels-1, image_size, image_size), cond_scale, rescaled_phi)#改
    ...
    def p_losses(self, x_start, condition,t,gt, *, classes,noise = None):#改def p_losses(self, x_start, t, *, classes, noise = None):
        b, c, h, w = x_start.shape  #x_strat 就是公式中的x0
        
        noise = default(noise, lambda: torch.randn_like(x_start))#生成随机噪声 噪声的shape就是图片的shape

        # noise sample

        x = self.q_sample(x_start = x_start, t = t, noise = noise)#加噪，得到t时刻的图片xt
        ...
        model_out = self.model(x, t, classes,condition)#改
        ...
        ### sample ??? 直接通过x0 和xt的关系得到x0
        #pred_xstart = self.sample(classes=classes,condition = condition,cond_scale=1)
        #alphas = self.alphas_cumprod[t].unsqueeze(1).unsqueeze(2).unsqueeze(3) 
        #pred_xstart = 1/torch.sqrt(alphas)*(x-torch.sqrt(1-alphas)*target)
       ...
       # warp_rs = get_warp_flow(condition.detach().cpu().numpy(), pred_xstart.detach().cpu().numpy(), start=0)
        ...
        """ condition_img = condition[0]
        #warp_rs = unnormalize_to_zero_to_one(warp_rs)
        warp_rs_img = warp_rs[0]
        #gt = unnormalize_to_zero_to_one(gt)
        gt_img = gt[0]

        xstart = unnorm_flow(target)
        
        warp_rs_img1 = flow_warp(condition, xstart, pad="zeros", mode="bilinear")
        #warp_rs_img1 = unnormalize_to_zero_to_one(warp_rs_img1)
        
        warp_rs_img1 = warp_rs_img1[0]
        print('flow_max')
        print(torch.max(xstart))
        print(torch.min(xstart))
        print(torch.max(pred_xstart))
        print(torch.min(pred_xstart))

        print('save_img1')
        utils.save_image(warp_rs_img, 'warp_rs_img.jpg')
        print(torch.max(warp_rs_img))
        print(torch.min(warp_rs_img))
        #time1.sleep(10)
        print('save_img2')
        utils.save_image(gt_img, 'warp_rs_img1.jpg')
        #time1.sleep(10)
        print('save_img3')
        utils.save_image(condition_img, 'warp_rs_img2.jpg')

        print('save_img4')
        utils.save_image(warp_rs_img1, 'warp_rs_img4.jpg')
        print(torch.max(warp_rs_img1))
        print(torch.min(warp_rs_img1))

        print('loss:',loss.sum())
        #time1.sleep(10)
        print('photoloss:',photo_loss.sum()/(photo_loss.sum()/loss.sum()).detach())
        print('photoloss1:',photo_loss.sum())
        #time1.sleep(10)
        
        flow1 = flow_to_image(pred_xstart[0].detach().cpu().numpy().transpose((1,2,0)))
        flow2 = flow_to_image(xstart[0].detach().cpu().numpy().transpose((1,2,0)))
        cv2.imwrite('warp_p_flow.jpg',flow1)
...
class test_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        show_rs = img_array[:,:800]
        show_rs = Image.fromarray(show_rs)
        #flow = torch.tensor(homo)/(self.image_size-1)
        show_rs = self.transform1(show_rs)
    
        show_gs =  img_array[:,800:]
        show_gs = Image.fromarray(show_gs)
        #flow = torch.tensor(homo)/(self.image_size-1)
        show_gs = self.transform1(show_gs)
        
        return [out_flow,rs,gs,show_rs,show_gs]
...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #out_flow = transformImage(homo,200,600,800)
        #out_flow = out_flow.transpose((2,0,1))
        #out_flow = out_flow[np.newaxis,:]
        #out_flow = resize_flow(out_flow,(self.image_size,self.image_size))
        #out_flow = out_flow.squeeze()
        """ out_flow = torch.tensor(out_flow, dtype=torch.float32)

        img = Image.open(path)
        img_array = np.array(img)
        #condtion = img_array[:, -800:]
        condition = img_array[:,:800]
        condition = Image.fromarray(condition)
        #flow = torch.tensor(homo)/(self.image_size-1)
        condition1 = self.transform(condition)
        ...
        return [self.transform(img),self.transform(condition),flow]
 """
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        self.test_ds = test_Dataset(test_folder, self.image_size, augment_horizontal_flip = augment_horizontal_flip, convert_image_to = convert_image_to)
        ...
        self.test_dl = DataLoader(self.test_ds, batch_size = self.batch_size, shuffle = False, pin_memory = True, num_workers = cpu_count() // 2)
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    ...
                    #gt_img = gs[0]
                    print('save_img11')
                    utils.save_image(warp_rs_img, 'warp_rs_img11.jpg')
                    #time1.sleep(10)
                    print('save_img22') """
                   
                    #gs = flow_warp(rs, flow, pad="zeros", mode="bilinear").to(device)
                    """ gt_img = gs[0]
                    utils.save_image(gt_img, 'warp_rs_img22.jpg')
                    #time1.sleep(10)
                    gs1 = flow_warp(rs, flow, pad="zeros", mode="bilinear").to(device)
                    gt_img1 = gs1[0]
                    utils.save_image(gt_img1, 'warp_rs_img33.jpg') """
                    ...
                    with self.accelerator.autocast():
                        loss = self.model(norm_flow1, classes=class1, condition=rs,gt = gs)  #改 loss = self.model(data, classes=class1)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and divisible_by(self.step, self.save_and_sample_every):
                        ...
                        #img1,condition1 = load_test('test1',self.image_size)
                        #img1 = torch.stack(img1, dim=0)
                        #condition1 = torch.stack(condition1, dim=0).to(device)
                        
                        with torch.inference_mode():
                            milestone = self.step // self.save_and_sample_every
                            batches = num_to_groups(self.num_samples, self.batch_size)  
                            #image_classes =  torch.zeros([self.batch_size]).cuda().to(torch.int64)
                            #out_flow = self.ema.ema_model.sample(classes=image_classes,condition = condition1,cond_scale=1)
                            
                            #all_images = flow_warp(condition1, out_flow, pad="zeros", mode="bilinear")
                            ...
                            #for idx in tqdm(range(self.test_long), total=self.test_long):
                                ...
                                ### show testimg
                                """ test_rs = rs[0]
                                utils.save_image(test_rs, 'test_rs.jpg')
                                gs = flow_warp(rs, flow, pad="zeros", mode="bilinear").to(device)
                                test_gs = gs[0]
                                utils.save_image(test_gs, 'test_gs.jpg') """
                                ...
                                for i in range(all_images.shape[0]):
                                    ...
                                    if not os.path.exists(str(self.results_folder / f'sample-{milestone}'/'image')):
                                        os.makedirs(str(self.results_folder / f'sample-{milestone}'/'image'))
                                    if not os.path.exists(str(self.results_folder / f'sample-{milestone}'/'flow')):
                                        os.makedirs(str(self.results_folder / f'sample-{milestone}'/'flow'))
                                    if not os.path.exists(str(self.results_folder / f'sample-{milestone}'/'pred_flow')):
                                        os.makedirs(str(self.results_folder / f'sample-{milestone}'/'pred_flow'))
                                    if not os.path.exists(str(self.results_folder / f'sample-{milestone}'/'true_flow')):
                                        os.makedirs(str(self.results_folder / f'sample-{milestone}'/'true_flow'))
                                        
                                    image_path = os.path.join(str(self.results_folder / f'sample-{milestone}'/'image'), f'{idx:05d}_image_{i:05d}.jpg')
                                    ...
                                    ture_flow_path1 = os.path.join(str(self.results_folder / f'sample-{milestone}'/'pred_flow'), f'{idx:05d}_flow_{i:05d}.npy')
                                    np.save(ture_flow_path1,flow1)
                                    ture_flow_path2 = os.path.join(str(self.results_folder / f'sample-{milestone}'/'true_flow'), f'{idx:05d}_flow_{i:05d}.npy')
                                    ...
                                    flow_path = os.path.join(str(self.results_folder / f'sample-{milestone}'/'flow'), f'{idx:05d}_flow_{i:05d}.jpg')
